...
test('handles scrim to bottom with pixel length', t => {
  return run(t,
    '.shadow { background-image: scrim(to bottom, 100px, #000); }',
...
test('handles scrim 45deg', t => {
  return run(t,
    '.shadow { background-image: scrim(45deg, 100%, #000); }',
...
test('handles multiple backgrounds with scrim to bottom', t => {
  return run(t,
    '.shadow { background: scrim(to bottom, 6em, #000), linear-gradient(to top, #fff, transparent 3em), #f00; }',
...
test('handles multiple scrims', t => {
  return run(t,
    '.shadow { background-image: scrim(to bottom, 100px, #000), scrim(to top, 100px, #000); }',
...
test('handles colored scrim', t => {
  return run(t,
    '.shadow { background: scrim(to bottom, 6em, red); }',
